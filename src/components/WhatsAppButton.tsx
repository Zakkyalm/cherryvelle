...
export function WhatsAppButton() {
  return (
    ...
    >
      ...
      <span className="absolute inset-0 rounded-full bg-[#25D366] animate-wa-blink" />
      {/* ripple ring 1 */}
      <span className="absolute inset-0 rounded-full bg-[#25D366] animate-wa-ring" />
      {/* ripple ring 2 – delayed */}
      <span className="absolute inset-0 rounded-full bg-[#25D366] animate-wa-ring" style={{ animationDelay: '0.8s' }} />
      <svg
        className="w-8 h-8 relative z-10 drop-shadow"
        fill="currentColor"
        viewBox="0 0 24 24"
